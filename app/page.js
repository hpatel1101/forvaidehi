...
export default function Page() {
  ...
  async function handleLookup(event) {
    ...
    try {
      ...
      if (parties.length === 1) {
        ...
        setLookupMessage('We found multiple possible matches. Please choose your party below.');
      }
    } catch (error) {
  ...
  async function handleSubmit(event) {
    event.preventDefault();
    if (!party) return setSubmitError('Please choose your party first.');
  ...
  return (
    <main>
      ...
      {matches.length > 1 ? <div className="match-list">{matches.map((match) => <button key={match.partyName} type="button" className={`match-card ${party && party.partyName === match.partyName ? 'active' : ''}`} onClick={() => chooseParty(match)}><span className="match-title">{match.partyName}</span><span className="match-subtitle">Matched: {match.matchedGuests.join(', ')}</span><span className="match-subtitle">Party: {match.members.map((m) => m.fullName).join(', ')}</span></button>)}</div> : null}
