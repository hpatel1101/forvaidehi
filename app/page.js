...
export default function Page() {
  ...
  const allSelected = useMemo(() => party ? party.members.every((m) => Boolean(statuses[m.id])) : false, [party, statuses]);

  async function handleLookup(event) {
    event.preventDefault();
    setLoading(true); setLookupError(''); setLookupMessage(''); setSubmitError(''); setSubmitMessage('');
    try {
      const response = await fetch(`/api/search?q=${encodeURIComponent(query)}`);
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Unable to search right now.');
      if (!data.party) { setParty(null); setStatuses({}); setLookupError('We could not find that guest. Try a full first and last name.'); return; }
      setParty(data.party);
      setStatuses(Object.fromEntries(data.party.members.map((m) => [m.id, m.attending || ''])));
  ...
  async function handleSubmit(event) {
    event.preventDefault();
    if (!party) return setSubmitError('Please find your party first.');
    if (!allSelected) return setSubmitError('Please choose attending or not attending for each guest.');
    setSaving(true); setSubmitError(''); setSubmitMessage('');
    try {
      const updates = party.members.map((m) => ({ id: m.id, attending: statuses[m.id] }));
      const response = await fetch('/api/rsvp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ updates }) });
      const data = await response.json();
